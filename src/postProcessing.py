...
def PostProcess():
    ...
    folder = "C:\\dev\\THERMOSS\\HP2Dt_1\\cpp\\results\\results_14-10-2020_15-03-47"

    while True:
        
        # 0.5_T2Dt.csv
        try:
            u = pd.read_csv('{}/{}_T2Dt.csv'.format(folder, str(n)),index_col=False,header=None, sep=';')
            u.drop(u.columns[len(u.columns)-1], axis=1, inplace=True)

            u_TEG_P = pd.read_csv('{}/{}_u_TEG_P.csv'.format(folder, str(n)),index_col=False,header=None, sep=';')
            u_TEG_P.drop(u_TEG_P.columns[len(u_TEG_P.columns)-1], axis=1, inplace=True)

            u_TEG_N = pd.read_csv('{}/{}_u_TEG_N.csv'.format(folder, str(n)),index_col=False,header=None, sep=';')
            u_TEG_N.drop(u_TEG_N.columns[len(u_TEG_N.columns)-1], axis=1, inplace=True)


            uLoo = pd.read_csv('{}/{}_uLoo.csv'.format(folder, str(n)),index_col=False,header=None, sep=';')
            uLoo.drop(uLoo.columns[len(uLoo.columns)-1], axis=1, inplace=True)

            uLcc = pd.read_csv('{}/{}_uLcc.csv'.format(folder, str(n)),index_col=False,header=None, sep=';')
            uLcc.drop(uLcc.columns[len(uLcc.columns)-1], axis=1, inplace=True)

            
            Q_exc = pd.read_csv('{}/{}_Q_exc.csv'.format(folder, str(n)),index_col=False,header=None, sep=';')
            Q_exc.drop(Q_exc.columns[len(Q_exc.columns)-1], axis=1, inplace=True)

            Qoo = pd.read_csv('{}/{}_Qoo.csv'.format(folder, str(n)),index_col=False,header=None, sep=';')
            Qoo.drop(Qoo.columns[len(Qoo.columns)-1], axis=1, inplace=True)

            Qcc = pd.read_csv('{}/{}_Qcc.csv'.format(folder, str(n)),index_col=False,header=None, sep=';')
            Qcc.drop(Qcc.columns[len(Qcc.columns)-1], axis=1, inplace=True)


            I = pd.read_csv('{}/{}_I.csv'.format(folder, str(n)),index_col=False,header=None, sep=';')
